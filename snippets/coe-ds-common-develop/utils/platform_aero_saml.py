"""
Setup AWS token via SAML authentication
"""
...
def platform_aero_saml():
    ...
    idpentryurl = 'https://fs.platform.aero/adfs/ls/IdpInitiatedSignOn.aspx?loginToRp=urn:amazon:webservices'
    ...
    conn = boto.sts.connect_to_region(region, validate_certs=sslverification, is_secure=sslverification)
    ...
    print('\n\n----------------------------------------------------------------')
    print('Your new access key pair has been stored in the AWS configuration file {0} under the saml profile.'.
          format(filename))
    ...
    s3conn = boto.s3.connect_to_region(region,
                                       ...
                                       validate_certs=sslverification)
